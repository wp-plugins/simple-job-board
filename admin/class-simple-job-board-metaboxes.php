...
class Simple_Job_Board_Metaboxes
{
    ...
    public function add_meta_boxes() {
        ...
        $disallowed = array( 'page', 'post', 'attachment', 'jobpost_applicants' );
        foreach ( $post_types as $post_type ) {
            if ( in_array( $post_type, $disallowed ) ) {
                continue;
            }
            $this->add_meta_box('post_options', __( 'Job Data' , 'simple-job-board' ), $post_type);
        }
    }

    public function add_meta_box( $id, $label, $post_type ) {
        add_meta_box( 'simple-job-board-' . $id, $label, array( $this, $id ), $post_type );
    }

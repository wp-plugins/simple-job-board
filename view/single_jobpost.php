                        ...
                        switch ($val['type']){
                            ...
                            case 'radio': echo '<div class="form-group"><label for="'.$key.'">'.str_replace('_',' ',substr($key,7)).'</label><div id="'.$key.'" >';
                                ...
                                foreach ($options as $option) {
                                    echo '<input type="radio" name="'.$key.'" class="" id="'.$key.'" value="'.$option.'"  '.jobpost_is_checked($i).'>'.$option.' &nbsp; &nbsp; ';
                                    $i++;
                                }
                                echo '</div></div>';
                                break;
                            ...
                            case 'checkbox' : echo '<div class="form-group"><label for="'.$key.'">'.str_replace('_',' ',substr($key,7)).'</label><div id="'.$key.'" >';
                                ...
                                $i=0;
                                foreach ($options as $option) {
                                    echo '<input type="checkbox" name="'.$key.'" class="" id="'.$key.'" value="'.$option.'"  '.jobpost_is_checked($i).'>'.$option.' &nbsp; &nbsp; ';
                                    $i++;
                                }
                                echo '</div></div>';
                                break;
...
function jobpost_features_frontend_list($content) {
    global $post;
...
add_filter( 'the_content', 'jobpost_features_frontend_list');
?>